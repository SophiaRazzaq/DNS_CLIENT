...
suite('Unflatten', function () {
  ...
  suite('Overwrite + non-object values in key positions', function () {
    test('non-object keys + overwrite should be overwritten', function () {
      assert.deepStrictEqual(flat.unflatten({
        a: null,
        'a.b': 'c'
      }, {
        overwrite: true
...
suite('CLI', function () {
  test('can take filename', function (done) {
    ...
    exec("".concat(cli, " ").concat(pkgJSON), function (err, stdout, stderr) {
      assert.ifError(err);
  ...
  test('can take piped file', function (done) {
    var cli = path.resolve(__dirname, '..', pkg.bin);
    var pkgJSON = path.resolve(__dirname, '..', 'package.json');
    exec("cat ".concat(pkgJSON, " | ").concat(cli), function (err, stdout, stderr) {
      assert.ifError(err);
      assert.strictEqual(stdout.trim(), JSON.stringify(flatten(pkg), null, 2));
      done();
    });
