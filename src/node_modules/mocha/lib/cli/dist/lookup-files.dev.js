...
module.exports = function lookupFiles(filepath) {
  ...
  if (!fs.existsSync(filepath)) {
    ...
    if (glob.hasMagic(filepath, {
      ...
      pattern = "".concat(filepath, "+(").concat(strExtensions, ")");
      debug('looking for files using glob pattern: %s', pattern);
    }

    files.push.apply(files, _toConsumableArray(glob.sync(pattern, {
      nodir: true,
      windowsPathsNoEscape: true
    })));

    if (!files.length) {
      throw createNoFilesMatchPatternError("Cannot find any files matching pattern \"".concat(filepath, "\""), filepath);
    }
  ...
  fs.readdirSync(filepath).forEach(function (dirent) {
    ...
    if (!extensions.length) {
      throw createMissingArgumentError("Argument '".concat(extensions, "' required when argument '").concat(filepath, "' is a directory"), 'extensions', 'array');
    }
