...
exports.watchParallelRun = function (mocha, _ref, fileCollectParams) {
  ...
  return createWatcher(mocha, {
    ...
    beforeRun: function beforeRun(_ref2) {
      ...
      var rootSuite = mocha.suite.clone(); // ensure we aren't leaking event listeners

      mocha.dispose(); // this `require` is needed because the require cache has been cleared.  the dynamic
      // exports set via the below call to `mocha.ui()` won't work properly if a
      // test depends on this module.

      var Mocha = require('../mocha'); // ... and now that we've gotten a new module, we need to use it again due
      // to `mocha.ui()` call


      var newMocha = new Mocha(mocha.options); // don't know why this is needed

      newMocha.suite = rootSuite; // nor this

      newMocha.suite.ctx = new Context(); // reset the list of files

      newMocha.files = collectFiles(fileCollectParams).files; // because we've swapped out the root suite (see the `run` inner function
      // in `createRerunner`), we need to call `mocha.ui()` again to set up the context/globals.

      newMocha.ui(newMocha.options.ui); // we need to call `newMocha.rootHooks` to set up rootHooks for the new
      // suite

      newMocha.rootHooks(newMocha.options.rootHooks); // in parallel mode, the main Mocha process doesn't actually load the
...
exports.watchRun = function (mocha, _ref3, fileCollectParams) {
  ...
  return createWatcher(mocha, {
    ...
    beforeRun: function beforeRun(_ref4) {
      ...
      mocha.unloadFiles(); // I don't know why we're cloning the root suite.
...
var createWatcher = function createWatcher(mocha, _ref5) {
  ...
  var globalFixtureContext; // we handle global fixtures manually
  ...
  watcher.on('ready', function _callee() {
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            _context.next = 4;
            return regeneratorRuntime.awrap(mocha.runGlobalSetup());
    ...
    return regeneratorRuntime.async(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            showCursor();
            console.error("".concat(logSymbols.warning, " [mocha] cleaning up, please wait..."));
            ...
            _context2.prev = 6;
            _context2.next = 9;
            return regeneratorRuntime.awrap(mocha.runGlobalTeardown(globalFixtureContext));
...
var createRerunner = function createRerunner(mocha, watcher) {
  var _ref6 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {},
      beforeRun = _ref6.beforeRun;

  // Set to a `Runner` when mocha is running. Set to `null` when mocha is not
  // running.
  var runner = null; // true if a file has changed during a test run

  var rerunScheduled = false;

  var run = function run() {
    try {
      mocha = beforeRun ? beforeRun({
        mocha: mocha,
        watcher: watcher
      }) || mocha : mocha;
      runner = mocha.run(function () {
        ...
        if (rerunScheduled) {
          ...
          console.error("".concat(logSymbols.info, " [mocha] waiting for changes..."));
        }
      });
    } catch (e) {
