...
var parse = function parse() {
  ...
  for (var _len = arguments.length, configObjects = new Array(_len > 2 ? _len - 2 : 0), _key = 2; _key < _len; _key++) {
  ...
  if (result.error) {
    console.error(ansi.red("Error: ".concat(result.error.message)));
    process.exit(1);
...
var loadOptions = function loadOptions() {
  ...
  var args = parse(argv); // short-circuit: look for a flag that would abort loading of options
  ...
  var envConfig = parse(process.env.MOCHA_OPTIONS || '');
  ...
  args = parse(args._, mocharc, args, envConfig, rcConfig || {}, pkgConfig || {}); // recombine positional arguments and "spec"
