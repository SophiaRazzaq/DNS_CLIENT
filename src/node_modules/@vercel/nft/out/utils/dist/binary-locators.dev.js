...
var graceful_fs_1 = __importDefault(require("graceful-fs")); // pregyp


var versioning = require('@mapbox/node-pre-gyp/lib/util/versioning.js');

var napi = require('@mapbox/node-pre-gyp/lib/util/napi.js');

var pregypFind = function pregypFind(package_json_path, opts) {
  var package_json = JSON.parse(graceful_fs_1["default"].readFileSync(package_json_path).toString());
  ...
  opts = opts || {};
  if (!opts.module_root) opts.module_root = path_1["default"].dirname(package_json_path);
...
function makeModulePathList(root, name) {
  return [[root, name], [root, 'build', name], [root, 'build', 'Debug', name], [root, 'build', 'Release', name], [root, 'out', 'Debug', name], [root, 'Debug', name], [root, 'out', 'Release', name], [root, 'Release', name], [root, 'build', 'default', name], [root, process.env['NODE_BINDINGS_COMPILED_DIR'] || 'compiled', process.versions.node, process.platform, process.arch, name]];
}
...
function nbind() {
  ...
  var found = findCompiledModule(basePath, [{
    ext: '.node',
    name: 'nbind.node',
    type: 'node'
  }, {
