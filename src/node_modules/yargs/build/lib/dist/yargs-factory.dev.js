...
exports.YargsWithShim = YargsWithShim;
...
function Yargs() {
  ...
  var y18n = shim.y18n;
  self.middleware = (0, _middleware.globalMiddlewareFactory)(globalMiddleware, self);
  ...
  if (shim.getEnv('_') && shim.getProcessArgvBin() === shim.getEnv('_')) {
    self.$0 = shim.getEnv('_').replace("".concat(shim.path.dirname(shim.process.execPath()), "/"), '');
  }
  ...
  self.resetOptions = self.reset = function resetOptions() {
    ...
    objectOptions.forEach(function (k) {
      tmpOptions[k] = (0, _objFilter.objFilter)(options[k], function (k) {
        return !localLookup[k];
      });
    });
    tmpOptions.envPrefix = options.envPrefix;
    options = tmpOptions;
    usage = usage ? usage.reset(localLookup) : (0, _usage.usage)(self, y18n, shim);
    validation = validation ? validation.reset(localLookup) : (0, _validation.validation)(self, usage, y18n, shim);
    command = command ? command.reset() : (0, _command.command)(self, usage, validation, globalMiddleware, shim);
    if (!completion) completion = (0, _completion.completion)(self, usage, command, shim);
  ...
  self["default"] = self.defaults = function (key, value, defaultDescription) {
    ...
    if (defaultDescription) {
      (0, _commonTypes.assertSingleKey)(key, shim);
  ...
  self.describe = function (key, desc) {
    (0, _argsert.argsert)('<object|string|array> [string]', [key, desc], arguments.length);
    setKey(key, true);
  ...
  function populateParserHintDictionary(builder, type, key, value, singleKeyHandler) {
    if (Array.isArray(key)) {
      ...
      try {
        for (var _iterator = (0, _commonTypes.objectKeys)(key)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var k = _step.value;
          builder(k, key[k]);
        }
      } catch (err) {
  ...
  function deleteFromParserHintObject(optionKey) {
    (0, _commonTypes.objectKeys)(options).forEach(function (hintKey) {
      if (function (key) {
  ...
  self.config = function config() {
    ...
    if (_typeof(key) === 'object' && !Array.isArray(key)) {
      key = (0, _applyExtends.applyExtends)(key, cwd, self.getParserConfiguration()['deep-merge-config'] || false, shim);
  ...
  self.demand = self.required = self.require = function demand(keys, max, msg) {
    if (Array.isArray(max)) {
      max.forEach(function (key) {
        (0, _commonTypes.assertNotStrictEqual)(msg, true, shim);
  ...
  self.usage = function (msg, description, builder, handler) {
    ...
    if (description !== undefined) {
      (0, _commonTypes.assertNotStrictEqual)(msg, null, shim);
  ...
  self.fail = function (f) {
    (0, _argsert.argsert)('<function>', [f], arguments.length);
    usage.failFn(f);
    return self;
  };
  ...
  self.check = function (f, _global) {
    (0, _argsert.argsert)('<function> [boolean]', [f, _global], arguments.length);
    validation.check(f, _global !== false);
    return self;
  };

  self.global = function global(globals, global) {
    (0, _argsert.argsert)('<string|array> [boolean]', [globals, global], arguments.length);
  ...
  self.pkgConf = function pkgConf(key, rootPath) {
    ...
    if (obj[key] && _typeof(obj[key]) === 'object') {
      conf = (0, _applyExtends.applyExtends)(obj[key], rootPath || cwd, self.getParserConfiguration()['deep-merge-config'] || false, shim);
  ...
  function pkgUp(rootPath) {
    ...
    try {
      ...
      (0, _commonTypes.assertNotStrictEqual)(pkgJsonPath, undefined, shim);
  ...
  self.positional = function (key, opts) {
    ...
    if (context.resets === 0) {
      throw new _yerror.YError(".positional() can only be called in a command's builder function");
    }
  ...
  self.getCompletion = function (args, done) {
    (0, _argsert.argsert)('<array> <function>', [args, done], arguments.length);
    completion.getCompletion(args, done);
  };

  self.locale = function (locale) {
    (0, _argsert.argsert)('[string]', [locale], arguments.length);
  ...
  var _logger = {
    log: function log() {
      ...
      if (!self._hasParseCallback()) (_console = console).log.apply(_console, args);
      hasOutput = true;
    ...
    error: function error() {
      ...
      if (!self._hasParseCallback()) (_console2 = console).error.apply(_console2, args);
      hasOutput = true;
      if (output.length) output += '\n';
      output += args.join(' ');
    }
    ...
    try {
      ...
      if (argv._.length) {
        ...
        if (completionCommand && ~argv._.indexOf(completionCommand) && !requestCompletions) {
          if (exitProcess) (0, _setBlocking["default"])(true);
      ...
      if (err instanceof _yerror.YError) usage.fail(err.message, err);else throw err;
    }
  ...
  self._parsePositionalNumbers = function (argv) {
    ...
    for (var i = 0, arg; (arg = args[i]) !== undefined; i++) {
      if (shim.Parser.looksLikeNumber(arg) && Number.isSafeInteger(Math.floor(parseFloat("".concat(arg))))) {
        args[i] = Number(arg);
      }
    }
...
var rebase = function rebase(base, dir) {
